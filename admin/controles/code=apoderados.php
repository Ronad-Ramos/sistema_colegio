...
if(isset($_POST['tipo']) AND $_POST['tipo'] != "" ){
	...
	if($_POST['tipo'] == "r"){
		...
		if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != "") {
			...
			if(in_array($imgExt, $valid_extensions)){   
			    ...
			    $resultado = $sentencia->execute(["4",$_POST['nombres'],$_POST['apellidos'],$_POST['correo'],$user,$_POST['password'],$_POST['nacimiento'],$img]); 
	...
	if($_POST['tipo'] == "m"){
		...
		$detallesUs = $conexion->prepare("SELECT * FROM usuarios WHERE ROL = 4");
        $detallesUs->execute();

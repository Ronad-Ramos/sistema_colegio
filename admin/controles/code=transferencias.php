...
if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != "" && $_FILES['file']['tmp_name'] != "") {
	...
	$upload_dir = '../../archivos/transferencias/'; // upload directory
	...
	if(in_array($imgExt, $valid_extensions)){ 
		...
		if(move_uploaded_file($tmp_dir,$upload_dir.$file)){
			...
			if(isset($info['ID']) && $info['ID'] !=""){
    			...
    			if ($infoAp > 0) {
 					if ($infoAl > 0) {
 						...
 						$sentencia = $conexion->prepare("INSERT INTO `transferencia`(`ID_TRANSFERENCIA`, `ID_USUARIO`, `ID_ALUMNO`, `ID_APODERADO`, `FECHA`, `ARCHIVO`) VALUES (NULL,?,?,?,NOW(),?)");
			...
			}else{ $message = 'Inicia sesion para registar una transferencia => <a href="auth.php">Aqui</<>'; $valor = '0'; }
...
}else{ $message = 'Selecione su documento de transferencia'; $valor = '0';}
	...
	if($_POST['tipo'] == "m"){
		...
		$detallesUs = $conexion->prepare("SELECT * FROM transferencia ORDER BY ID_TRANSFERENCIA DESC");
        $detallesUs->execute();
		...
		foreach($dagta as $matri){
			...
			$valor .='<tr>';
			$valor .='	<th scope="row">'.$matri['ID_TRANSFERENCIA'].'</th>';
			...
			$valor .='		<button id="btnEditar'.$matri['ID_TRANSFERENCIA'].'" type="button" onclick="editarRetiro('.$matri['ID_TRANSFERENCIA'].');" class="waves-effect waves-circle btn btn-circle btn-success mb-5"><i class="mdi mdi-lead-pencil"></i></button>';
			$valor .='		<button id="btnEliminar'.$matri['ID_TRANSFERENCIA'].'" type="button" onclick="eliminarRetiro('.$matri['ID_TRANSFERENCIA'].');" class="waves-effect waves-circle btn btn-circle btn-danger mb-5"><i class="mdi mdi-close-circle"></i></button>';
			$valor .='	</td>';
			$valor .='</tr>';
		}
	...
	if($_POST['tipo'] == "a"){
		...
		$detallesU = $conexion->prepare("SELECT * FROM transferencia WHERE ID_TRANSFERENCIA =:d");
    	$detallesU -> bindParam(':d', $_POST['idd']);
    	$detallesU->execute();

    	$transferencia = $detallesU->fetch(PDO::FETCH_ASSOC);
		...
		if (isset($_POST['nombres_apoderado_ac']) AND $_POST['nombres_apoderado_ac'] != "") {
            ...
            $resultado = $sentencia->execute([$_POST['nombres_apoderado_ac'],$transferencia['ID_APODERADO']]);
		...
		if (isset($_POST['apellidos_apoderado_ac']) AND $_POST['apellidos_apoderado_ac'] != "") {
            ...
            $resultado = $sentencia->execute([$_POST['apellidos_apoderado_ac'],$transferencia['ID_APODERADO']]);
            $message = '<p>Información actualizada correctamente</p>'; $valor = '1';
		}
		//alumno
		if (isset($_POST['nombres_alumno_ac']) AND $_POST['nombres_alumno_ac'] != "") {
			...
			$sentencia = $conexion->prepare("UPDATE `usuarios` SET `NOMBRES`=? WHERE `ID`=?");
            $resultado = $sentencia->execute([$_POST['nombres_alumno_ac'],$transferencia['ID_ALUMNO']]);
            $message = '<p>Información actualizada correctamente</p>'; $valor = '1';
		}
		...
		if (isset($_POST['apellidos_alumno_ac']) AND $_POST['apellidos_alumno_ac'] != "") {
			...
			$sentencia = $conexion->prepare("UPDATE `usuarios` SET `APELLIDOS`=? WHERE `ID`=?");
            $resultado = $sentencia->execute([$_POST['apellidos_alumno_ac'],$transferencia['ID_ALUMNO']]);
            $message = '<p>Información actualizada correctamente</p>'; $valor = '1';
		}
		...
		if($_POST["nivel_alumno_ac"] != "" AND $_POST["grado_alumno_ac"] !="" AND $_POST["seccion_alumno_ac"] !=""){
		    ...
		    if($_POST["nivel_alumno_ac"]==1){$n="primaria";}else{$n="secundaria";}
            	...
            	$resultado = $sentencia->execute([$idG,$transferencia['ID_ALUMNO']]);
           		$message = '<p>Información actualizada correctamente</p>'; $valor = '1';
	...
	if($_POST['tipo'] == "e"){ 
		...
		$detallesUs = $conexion->prepare("SELECT * FROM transferencia WHERE ID_TRANSFERENCIA = :u ");
		$detallesUs -> bindParam(':u', $_POST['id']); 
		$detallesUs -> execute();
		$transferencia = $detallesUs->fetch(PDO::FETCH_ASSOC);

		$sentencia = $conexion->prepare("DELETE FROM `transferencia` WHERE ID_TRANSFERENCIA = :i");
        $resultado = $sentencia->execute([ ':i' => $_POST['id'] ]);

		unlink('../../archivos/transferencias/'.$transferencia['ARCHIVO']);  
	...
	if($_POST['tipo'] == "sue"){
    	...
    	$detallesApod -> bindParam(':d', $transferencia['ID_APODERADO'], PDO::PARAM_STR);
    	...
    	$detallesAlum -> bindParam(':d', $transferencia['ID_ALUMNO'], PDO::PARAM_STR);
    	...
    	$return_arr[] = array("infoApoderado" => $infoApoderado , "infoAlumno" => $infoAlumno, "idMatri" => $transferencia['ID_TRANSFERENCIA'], "grado" => $infoGrado);
        echo json_encode($return_arr);
	}
